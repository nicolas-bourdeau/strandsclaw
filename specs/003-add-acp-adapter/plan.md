...
**Performance Goals**: Using the same local workstation and workspace as CLI validation, ACP initialization completes within 2 seconds of current chat startup, and each supported turn persists exactly one final assistant response  
...
- Define actionable error behavior for bootstrap failure, unreadable persisted session recovery, unsupported ACP operations, model unavailability, and disconnect-before-final-response handling.
- Define the minimum structured runtime events needed to keep ACP capability advertisement, session binding, and turn execution inspectable.
...
- Define observable ACP event names and payload boundaries for capability advertisement, session lifecycle, and turn outcomes.
...
- Slice C (P2): Bind ACP protocol sessions to the existing shared workspace session while preserving bootstrap, file-scope, disconnect, and persisted-session behavior.
- Slice D (P2): Add actionable ACP error mapping, structured runtime events, and explicit unsupported-capability handling for non-MVP protocol features.
